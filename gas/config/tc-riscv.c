...
riscv_multi_subset_supports (enum riscv_insn_class insn_class)
{
  switch (insn_class)
    ...
    case INSN_CLASS_HWACHA: return riscv_subset_supports("xhwacha");
...
static bfd_boolean riscv_sv_auto = FALSE;
	  ...
	  bfd_reloc_code_real_type *imm_reloc, struct hash_control *hash)
{
  ...
  if (insn == NULL)
  {
    ...
    if ((insn = (struct riscv_opcode *) hash_find (hash, str)) == NULL)
	  {
      /* Restore the character we overwrite above (if any).  */
      if (save_c)
        *(--s) = save_c;
      ...
      for (args = insn->args;; ++args)
	{
      ...
      case 'd':
        if (riscv_sv_auto) {
          ...
          if (EXTRACT_OPERAND( VD, ip->insn_opcode)) {
            if (!reg_lookup( &s, RCLASS_VEC_GPR, &regno ) )
      ...
      case 's':
        if (riscv_sv_auto) {
          ...
          if (EXTRACT_OPERAND( VS1, ip->insn_opcode)) {
            if (!reg_lookup( &s, RCLASS_VEC_GPR, &regno ) )
      ...
      case 'u':
        if (riscv_sv_auto) {
          if (!reg_lookup( &s, RCLASS_VEC_GPR, &regno ) ) {
            ...
            INSERT_OPERAND( VS1, *ip, 1);
            INSERT_OPERAND( VS2, *ip, 1);
          }
        } else {
          if (EXTRACT_OPERAND( VS1, ip->insn_opcode)) {
            if (!reg_lookup( &s, RCLASS_VEC_GPR, &regno ) )
      ...
      case 't':
        if (riscv_sv_auto) {
          ...
          if (EXTRACT_OPERAND( VS2, ip->insn_opcode)) {
            if (!reg_lookup( &s, RCLASS_VEC_GPR, &regno ) )
      ...
      case 'r':
        if (riscv_sv_auto) {
          ...
          if (EXTRACT_OPERAND( VS3, ip->insn_opcode)) {
            if (!reg_lookup( &s, RCLASS_VEC_GPR, &regno ) )
